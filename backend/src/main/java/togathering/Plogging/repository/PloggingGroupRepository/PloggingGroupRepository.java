...
public interface PloggingGroupRepository extends JpaRepository<PloggingGroup, Long> {
    ...
    List<PloggingGroup> findByType(PloggingGroupType type);

    List<PloggingGroup> findByNameContaining(String groupName);

    List<PloggingGroup> findByUserPloggingGroupApplymentsUser(User user);
}
